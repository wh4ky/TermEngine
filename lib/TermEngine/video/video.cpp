...
void TE::termScreen::cellSet(int x, int y, std::string new_value)
{
    ...
    // throw std::runtime_error("Error in \"TE::termScreen::cellSet\": could not replace old cell with new value.");
}
...
{
    video = new std::vector<std::vector<std::string>>(x_width, std::vector<std::string>(y_height));
}

void TE::termScreen::deAlloc()
{
    delete video;
}
